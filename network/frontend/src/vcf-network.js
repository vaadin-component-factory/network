...
class VcfNetwork extends ElementMixin(ThemableMixin(PolymerElement)) {
  ...
  set scale(value) {
    this._scale = value;
    this._restoreZoom();
    this.dispatchEvent(new CustomEvent('vcf-network-scale-changed', {detail: { scale: value }}));
  }
...
//  }
  ...
  _addToDataSet(dataset, items) {
    ...
    if (!Array.isArray(items)) {
      if (dataset === "nodes") {
        this.dispatchEvent(new CustomEvent('vcf-network-new-node', {detail: {node: items}}));
      } else if (dataset === "edges") {
        this.dispatchEvent(new CustomEvent('vcf-network-new-edge', {detail: {edge: items}}));
      }
    }
    if (this.context) {
      if (Array.isArray(items)) {
  ...
  _removeFromDataSet(dataset, items) {
    // JCG test return value
    const evt = this.dispatchEvent(new CustomEvent('vcf-network-before-delete-nodes', {detail: {nodes: items}, cancelable: true }));
    if (evt){
  ...
  _confirmRemoveFromDataSet(dataset, items) {
    ...
    if (items.length > 0) {
      if (dataset === "nodes") {
        this.dispatchEvent(new CustomEvent('vcf-network-after-delete-nodes', {detail: {nodes: items}}));
      } else if (dataset === "edges") {
        this.dispatchEvent(new CustomEvent('vcf-network-after-delete-edges', {detail: {edges: items}}));
      }
    }
    if (this.context) {
      if (Array.isArray(items)) {
  ...
  deleteFromDataSet(nodes, edges) {
    // JCG test return value
    const evt = this.dispatchEvent(new CustomEvent('vcf-network-delete', {detail: {nodes: nodes, edges : edges}, cancelable: true }));
    if (evt){
      // client side
      this.confirmDeleteFromDataSet(nodes, edges);
      this.dispatchEvent(new CustomEvent('vcf-network-after-delete', {detail: {nodes: nodes, edges: edges}}));
    }
  }

  confirmDeleteFromDataSet(nodes, edges) {
    this.data['nodes'].remove(nodes);
    this.data['edges'].remove(edges);
    if (this.context) {
      if (Array.isArray(nodes)) {
        this.context.component['nodes'] = this.context.component['nodes'].filter(node => !nodes.includes(node.id));
      } else {
        this.context.component['nodes'].splice(this.context.component['nodes'].indexOf(nodes.id), 1);
      }
      if (Array.isArray(edges)) {
        this.context.component['edges'] = this.context.component['edges'].filter(edge => !edges.includes(edge.id));
        this.context.component['edges'] = this.context.component['edges'].filter(edge => !edges.includes(edge.id));
      } else {
        this.context.component['edges'].splice(this.context.component['edges'].indexOf(edges.id), 1);
      }
  ...
  afterDeleteFomDataSet(nodes, edges) {
    this.dispatchEvent(new CustomEvent('vcf-network-after-delete', {detail: {nodes: nodes, edges: edges}}));
  }
